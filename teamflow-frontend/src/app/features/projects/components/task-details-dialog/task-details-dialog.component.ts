...
export class TaskDetailsDialogComponent implements OnInit {
    ...
    constructor(
        ...
        this.newSubTaskForm = this.fb.group({ title: [''] });
        this.newCommentForm = this.fb.group({ content: [''] });
    ...
    assignMember(member: Membership): void {
        this.taskService.assignMember(this.task.id, member.userId, 'CONTRIBUTOR').subscribe({
            next: (assignment) => {
    ...
    openEditDialog(): void {
        ...
        dialogRef.afterClosed().subscribe(result => {
            if (result) this.dialogRef.close({ refreshNeeded: true });
        });
    }
